<script>
  ...
  if (col.truePos + col.falsePos > maxY) {
    let newMax = col.truePos + col.falsePos;
...
<rect
  ...
  x={0}
  y={y(col.falseNeg + col.falsePos + truePos)}
  width={config.circleRadius * 2}
  height={y(col.falsePos + truePos) - y(col.falseNeg + col.falsePos + truePos)}
