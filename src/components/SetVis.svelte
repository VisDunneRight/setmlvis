<script lang="ts">
  ...
  import {IOU, dataset, height, selectedCol} from '../stores';
  ...
  const config = { colGap:3, 
                   maxTextSize:120,
                   ytickStep: 10,
                   circleRadius:9,
                   circleGap:3};
  // let winWidth = $windowWidth - $menuWidth;
  let winHeight = $height*.6;
...
for (let i = extentY[0]; i < Math.ceil(extentY[1]/config.ytickStep) + 1; i += 1) {
  yTicks.push(i * config.ytickStep);
}

//Helper functions
function columnSpacing(i:number){
  return padding.left + config.maxTextSize 
        + config.colGap + config.circleRadius
        + i * (config.colGap + config.circleRadius*2);
}
function modelRow(j:number){
  return - padding.bottom + winHeight 
      ...
      <g class="model-names">
        {#each metaModel as modelName, i}
          ...
          <text
            ...
            class="model-text">{modelName}</text>
        {/each}
      </g>
      <g class="y-axis">
        {#each yTicks as tick}
          <g class="tick tick-{tick}" 
             transform="translate({columnSpacing(0) - config.circleRadius - config.circleGap}, {y(tick)})">
            <line x2="100%"></line>
