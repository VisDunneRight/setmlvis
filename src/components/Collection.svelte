<script lang="ts">
  ...
  function rescaleImages(imageData:ImgData[],
    ...
    imageData.forEach((img)=>{
      ...
      if(currentWidth + width + gap <= maxWidth){
        currentRow.push({top: top, left:left, data:img, height: startingHeight});
        left += width + gap;
        ...
        currentRow.forEach((item)=>{
          ...
          item.left = newLeft;
          let imgRat = item.data === undefined ? 1 : item.data.imgSize[0]/item.data.imgSize[1];
  ...
  $:imgInfo = rescaleImages($selectedCol, imgHeight, $windowWidth - $menuWidth - 8, gap);
</script>
